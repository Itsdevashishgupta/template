...
export const productsData=leadsdata
...
const meetingSources = ['Zoom', 'Microsoft Teams', 'Google Meet', 'Skype'];
const quantity=['23','25','33','45'];

for (let i = 0; i < ordersData.length; i++) {
    ordersData[i].project_name = projects[i % projects.length];
    ordersData[i].meetingSource = meetingSources[i % meetingSources.length];
    ordersData[i].quantity = quantity[i % quantity.length];
}
